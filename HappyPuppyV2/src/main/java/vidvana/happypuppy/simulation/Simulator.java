...
public class Simulator {
    ...
    static final int MIN_FACTS = 1000;
    public static int PROCESSES_PER_DAY;
            ...
            csvWriters.put(s, new CSVWriter(s, "T1_", "T2_", true,
                    (s.endsWith("EXCEL") ?
                            (s.startsWith("animals")?
                                    ...
                                    : "ID|PESEL|Volunteer's name|Volunteer's surname|Date of birth|Has experience" )
                            : null))
            );
            filenamePrefixes.add(prefix);
        }
        for (int i = 0; i < MIN_ANIMALS; i++) {
    ...
    public void simulate(int days, String prefix) {
        PROCESSES_PER_DAY = MIN_FACTS/days;
    ...
    public int getPeopleSize() { return people.size(); }
